...
const Quiz: React.FC = () => {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-12 animate-fade-in">
      <h1 className="text-4xl font-bold text-black text-center uppercase tracking-tight leading-none">Analyst Certification</h1>
      ...
      {showScore ? (
        <div className="bg-white rounded-sm shadow-lg border border-[#E0E2E5] p-16 text-center">
          ...
          <h2 className="text-3xl font-bold mb-4 uppercase text-black">You scored {score} out of {quizQuestions.length}</h2>
          <p className="text-[#595E62] mb-12 text-lg">
            {score === quizQuestions.length 
              ? "Perfect! You are a Query Engineer Master." 
              : score > quizQuestions.length / 2 
              ? "Good job! Review the Cheat Sheet to reach 100%." 
              : "Review the learning modules and try again."}
          </p>
          <button 
            onClick={resetQuiz}
          ...
          >
            <RefreshCw size={18} className="mr-3" /> Restart Quiz
          </button>
        </div>
      ) : (
        <div className="bg-white rounded-sm shadow-sm border border-[#E0E2E5] overflow-hidden">
          ...
          <div className="p-10">
            <div className="mb-8">
              <span className="text-xs font-bold text-[#979EA4] tracking-widest uppercase">Question {currentQuestion + 1}/{quizQuestions.length}</span>
            ...
            {isAnswered && (
              <div className="mt-8 pt-8 border-t border-[#E0E2E5] animate-fade-in flex flex-col items-end">
                <p className="text-[#595E62] mb-6 bg-[#F8FAFC] p-6 rounded-sm text-sm w-full border-l-4 border-[#6706EF]">
                  <span className="font-bold block mb-2 text-black uppercase text-xs tracking-wider">Explanation</span>
