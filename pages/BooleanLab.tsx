...
const BooleanLab: React.FC = () => {
  ...
  const [feedback, setFeedback] = useState<FeedbackState>({ status: 'idle', message: 'Enter a query to test effectiveness.', score: 0 });
  const [activeTab, setActiveTab] = useState<'editor' | 'examples'>('editor');

  const analyzeQuery = (q: string) => {
    const lowerQ = q.toLowerCase();
    
    // Logic remains the same, visuals update
    if ((lowerQ.includes('"ev"') || lowerQ.includes(' ev ')) && !lowerQ.includes('near') && !lowerQ.includes('electro-voice')) {
      return setFeedback({
        status: 'error',
        message: 'CRITICAL: Searching for "EV" without NEAR context retrieves Tesla noise. Precision: < 5%.',
        score: 10
      });
    }
    if (lowerQ.includes('battery') && !lowerQ.includes('not')) {
      if (lowerQ.includes('near')) {
         return setFeedback({
          status: 'warning',
          message: 'WARNING: "Battery" is risky. Ensure you use NEAR/x to link it to "Speaker".',
          score: 60
        });
      }
    }
    if (lowerQ.includes('near/') && (lowerQ.includes('speaker') || lowerQ.includes('mic') || lowerQ.includes('amp'))) {
       return setFeedback({
        status: 'success',
        message: 'EXCELLENT: Contextual Anchor detected. Linking brand to audio terms.',
        score: 95
      });
    }
    if (lowerQ.includes('dynacord') && lowerQ.includes('not') && lowerQ.includes('tape')) {
      return setFeedback({
        status: 'success',
        message: 'GREAT: Tracking Dynacord while excluding vintage tape echo noise.',
        score: 90
      });
    }
    if (q.length === 0) {
      return setFeedback({ status: 'idle', message: 'Start typing to see real-time feedback...', score: 0 });
    }
    return setFeedback({
      status: 'neutral',
      message: 'Remember the 3 steps: Inclusions, Context (NEAR/x), and Exclusions.',
      score: 40
    });
  };
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="text-4xl font-bold text-black uppercase tracking-tight leading-none mb-2">Boolean Laboratory</h1>
          <p className="text-[#595E62]">Test your query logic against the "Automotive Noise" filter simulation.</p>
        </div>
      </div>
              ...
              {activeTab === 'editor' ? (
                <>
                  ...
                  <div className="bg-[#000000] p-4 flex justify-end border-t border-[#1A1C1D]">
                    ...
                    >
                      <RefreshCcw size={12} /> Clear Console
                    </button>
                  </div>
                </>
              ) : (
                <div className="h-64 overflow-y-auto p-6 space-y-4 bg-white">
                  <div className="p-4 bg-[#F8FAFC] border-l-4 border-[#ED1C24]">
                    <p className="text-[10px] font-bold text-[#979EA4] uppercase mb-2">The Gold Standard</p>
                  ...
                  <div className="p-4 bg-[#F8FAFC] border-l-4 border-[#6706EF]">
                    <p className="text-[10px] font-bold text-[#979EA4] uppercase mb-2">Dynacord Clean</p>
                    <code className="text-xs font-mono text-black block break-words">
            ...
            <div className="flex items-start gap-6">
              ...
              <div>
                <h3 className="font-bold text-xl uppercase tracking-tight text-black mb-1">
                  Precision Score: <span className={feedback.status === 'error' ? 'text-[#ED1C24]' : 'text-black'}>{feedback.score}%</span>
                </h3>
        ...
        <div className="space-y-6">
          <div className="bg-[#000000] p-8 rounded-sm text-white relative overflow-hidden">
            ...
            <h3 className="font-bold uppercase tracking-tight mb-6 flex items-center gap-3">
              <Info size={20} className="text-[#ED1C24]"/> Logic Gates
            </h3>
            <ul className="space-y-4 text-sm text-[#E0E2E5]">
              <li className="flex flex-col gap-1">
                <span className="font-mono text-[#ED1C24] font-bold">NEAR/x</span>
                <span className="opacity-80">The "Sliding Window". Critical for "EV".</span>
              </li>
              <li className="flex flex-col gap-1">
                <span className="font-mono text-[#ED1C24] font-bold">NOT</span>
                <span className="opacity-80">The Guardrail. Excludes specific noise.</span>
              </li>
              <li className="flex flex-col gap-1">
                <span className="font-mono text-[#ED1C24] font-bold">""</span>
                <span className="opacity-80">Exact phrase match. Essential for "Live Sound".</span>
              </li>
            </ul>
          </div>

          <div className="bg-white p-8 rounded-sm border border-[#E0E2E5]">
            <h3 className="font-bold text-[#ED1C24] uppercase tracking-wider text-xs mb-3">Pro Tip</h3>
            <h4 className="font-bold text-black uppercase mb-2">The Battery Paradox</h4>
            <p className="text-sm text-[#595E62] leading-relaxed">
              EV cars use batteries (Noise). EVERSE speakers use batteries (Signal). 
              <br/><br/>
              Do NOT globally exclude "Battery". Use: <br/>
