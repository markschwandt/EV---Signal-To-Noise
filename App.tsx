        ...
        <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-[#000000] text-white transform transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0`}>
          <div className="flex flex-col h-full border-r border-[#1A1C1D]">
            ...
            <div className="px-6 flex-1">
               <p className="text-[10px] font-bold text-[#979EA4] uppercase tracking-widest mb-4 pl-4">Training Modules</p>
               <nav className="space-y-1">
                 <NavItem to="/" icon={<LayoutDashboard size={20}/>} label="Overview" />
                 <NavItem to="/visuals" icon={<Map size={20}/>} label="Visual Blueprints" />
                 <NavItem to="/noise" icon={<ShieldAlert size={20}/>} label="The EV Noise Problem" />
                 <NavItem to="/lab" icon={<Search size={20}/>} label="Boolean Lab" />
                 <NavItem to="/cheatsheet" icon={<Database size={20}/>} label="Taxonomy Reference" />
                 <NavItem to="/quiz" icon={<GraduationCap size={20}/>} label="Certification Quiz" />
               </nav>
            </div>
            ...
            <div className="mt-auto p-8 bg-gradient-to-t from-[#101112] to-transparent">
              <div className="flex items-center gap-4 border-t border-[#2E3033] pt-6">
                ...
                <div>
                  <p className="text-sm font-bold uppercase tracking-wide">Query Engineer</p>
                  <p className="text-xs text-[#979EA4]">Internal Tool v2.4</p>
                </div>
              </div>
            </div>
          </div>
